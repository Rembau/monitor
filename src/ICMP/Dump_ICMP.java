 ...
 *
 */
class Dump_ICMP implements PacketReceiver {
	private TreeSet<String> ts=new TreeSet<String>(); 
	private String localHost;
	Dump_ICMP(NetworkInterface networkInterface){
	...
	public void receivePacket(Packet packet) {
		...
		if(packet instanceof ICMPPacket){
			...
			if(Transition.byteToString(((IPPacket)packet).dst_ip).equals(this.localHost) && ((ICMPPacket)packet).code==0){
